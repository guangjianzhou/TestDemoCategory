...
- (ZXResultPoint *)findCornerFromCenter:(int)centerX deltaX:(int)deltaX left:(int)left right:(int)right centerY:(int)centerY deltaY:(int)deltaY top:(int)top bottom:(int)bottom maxWhiteRun:(int)maxWhiteRun {
  ...
  for (int y = centerY, x = centerX; y < bottom && y >= top && x < right && x >= left; y += deltaY, x += deltaX) {
    ...
    if (range == nil) {
      ...
      if (deltaX == 0) {
        int lastY = y - deltaY;
        if ([(NSNumber *)lastRange[0] intValue] < centerX) {
          if ([(NSNumber *)lastRange[0] intValue] > centerX) {
            return [[ZXResultPoint alloc] initWithX:deltaY > 0 ? [(NSNumber *)lastRange[0] intValue] : [(NSNumber *)lastRange[1] intValue] y:lastY];
          }
          return [[ZXResultPoint alloc] initWithX:[(NSNumber *)lastRange[0] intValue] y:lastY];
        } else {
          return [[ZXResultPoint alloc] initWithX:[(NSNumber *)lastRange[1] intValue] y:lastY];
        }
      } else {
        int lastX = x - deltaX;
        if ([(NSNumber *)lastRange[0] intValue] < centerY) {
          if ([(NSNumber *)lastRange[1] intValue] > centerY) {
            return [[ZXResultPoint alloc] initWithX:lastX y:deltaX < 0 ? [(NSNumber *)lastRange[0] intValue] : [(NSNumber *)lastRange[1] intValue]];
          }
          return [[ZXResultPoint alloc] initWithX:lastX y:[(NSNumber *)lastRange[0] intValue]];
        } else {
          return [[ZXResultPoint alloc] initWithX:lastX y:[(NSNumber *)lastRange[1] intValue]];
        }
      }
    }
    lastRange = range;
