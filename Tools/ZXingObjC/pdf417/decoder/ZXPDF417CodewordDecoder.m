...
+ (NSArray *)sampleBitCounts:(NSArray *)moduleBitCount {
  ...
  for (int i = 0; i < ZX_PDF417_MODULES_IN_CODEWORD; i++) {
    ...
    if (sumPreviousBits + [(NSNumber *)moduleBitCount[bitCountIndex] intValue] <= sampleIndex) {
      sumPreviousBits += [(NSNumber *)moduleBitCount[bitCountIndex] intValue];
      bitCountIndex++;
    }
    result[bitCountIndex] = @([(NSNumber *)result[bitCountIndex] intValue] + 1);
  }
  return result;
}
...
+ (int)bitValue:(NSArray *)moduleBitCount {
  ...
  for (int i = 0; i < [moduleBitCount count]; i++) {
    for (int bit = 0; bit < [(NSNumber *)moduleBitCount[i] intValue]; bit++) {
...
+ (int)closestDecodedValue:(NSArray *)moduleBitCount {
  ...
  for (int i = 0; i < ZX_PDF417_BARS_IN_MODULE; i++) {
    bitCountRatios[i] = [(NSNumber *)moduleBitCount[i] intValue] / (float) bitCountSum;
  }
