...
// general appearance
static const CGFloat CSToastMaxWidth            = 0.79;     // 79% of parent view width
static const CGFloat CSToastMaxHeight           = 0.8;      // 80% of parent view height
static const CGFloat CSToastHorizontalPadding   = 18.0;
...
+ (void)showToastWithText:(NSString *)text
{
    dispatch_async(dispatch_get_main_queue(), ^{
        [[UIApplication sharedApplication].keyWindow makeToast:text duration:0.7f position:@"bottom"];
    });
}
...
{
    dispatch_async(dispatch_get_main_queue(), ^{
        [[UIApplication sharedApplication].keyWindow showToast:view duration:0.7f position:@"bottom"];
    });
}
...
//            return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2)) - CSToastVerticalPadding);
            return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2)) - 60.f);
